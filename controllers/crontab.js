...
const db = new Datastore({filename: __dirname + '/crontabs/crontab.db'});
let cronPath = "/tmp";
...
let log_folder;
let env_file;
log_folder = __dirname + '/crontabs/logs';
env_file = __dirname + '/crontabs/env.db';

exports.log_folder = log_folder;
...
exports.create_new = function(name, command, schedule, logging, mailing){
    const tab = crontab(name, command, schedule, false, logging, mailing);
...
exports.crontabs = function(callback){
    db.find({}).sort({ created: -1 }).exec(function(err, docs){
        import_cron_log(docs,function(docs) {
    ...
    exports.crontabs( function(tabs){
        ...
        tabs.forEach(function(tab){
            if(!tab.stopped) {
                ...
                crontab_string += tab.schedule + " (((( " + tab.command + " )))) 3>&1 1>&2 2>&3 > " + stdout + " | tee " + stderr;
                ...
                if (tab.logging && tab.logging == "true") {
                    crontab_string += "; if [ -s " + stderr + " ]" +
                        "; then echo \"\\nCronRun @ Error\" >> " + log_file +
                        "; date >> " + log_file +
                        "; cat " + stderr + " >> " + log_file +
                        "; else echo \"\\nCronRun @ Success\" >> "  + log_file +
                        "; date >> " + log_file +
                        "; cat " + stdout + " >> " + log_file +
                        "; fi";
                }
...
exports.import_crontab = function(){
    exec("crontab -l", function(error, stdout, stderr){
        ...
        lines.forEach(function(line, index){
            ...
            regex = /^((\@[a-zA-Z]+\s+)|(([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)\s+))/;
...
import_cron_log = function(docs, callback){

    let files = fs.readdirSync(log_folder + "/");
    ...
    for(let i=0; i<docs.length; i++) {
        ...
        if(docs[i].logging && docs[i].logging != "false") {

            if(files.includes(docs[i]._id + ".log")) {

                lines = fs.readFileSync(log_folder + "/" + docs[i]._id + ".log", 'utf8');

                lines = lines.split("\n");

                lines.forEach(function (line, index) {
                    if (line.search("CronRun @") > -1) {
                        lastIndex = index;
                    }
                });
            }
        ...
        if (lastIndex === -1) {
            docs[i]["log_message"] = null;
            docs[i]["log_time"] = null;
        } else {
            docs[i]["log_message"] = lines[lastIndex];
            docs[i]["log_time"] = lines[lastIndex + 1];
        }
    }
